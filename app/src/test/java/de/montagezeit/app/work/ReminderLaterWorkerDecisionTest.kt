...
class ReminderLaterWorkerDecisionTest {
    ...
    @Test
    fun `MORNING snooze is suppressed for OFF entry`() = runTest {
    ...
    @Test
    fun `EVENING snooze is suppressed when evening snapshot already exists`() = runTest {
        val date = LocalDate.of(2026, 3, 12)
    ...
    @Test
    fun `FALLBACK snooze is suppressed when work day is already complete`() = runTest {
        val date = LocalDate.of(2026, 3, 12)
    ...
    @Test
    fun `DAILY snooze is suppressed for COMP_TIME entry`() = runTest {
        val date = LocalDate.of(2026, 3, 12)
        coEvery { workEntryDao.getByDate(date) } returns workEntry(
    ...
    @Test
    fun `snooze is suppressed for auto off day without entry`() = runTest {
    ...
    @Test
    fun `unknown reminder type is suppressed instead of falling back to morning`() = runTest {
        val date = LocalDate.of(2026, 3, 12)
        coEvery { workEntryDao.getByDate(date) } returns workEntry()
